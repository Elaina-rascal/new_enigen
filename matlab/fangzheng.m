...
Te_max = max(abs(Te));              % 最大电磁转矩 (N·m)
Te_nominal = 12000;                 % 假设额定转矩为 12000
Load_Factor = Te_max / Te_nominal;  % 载荷系数 (相对于额定)
...
fprintf('\n=== 仿真成绩单 ===\n');
fprintf('▶ 最大传动链载荷 (T_max): %.2f N·m (额定占比: %.1f%%)\n', Te_max, Load_Factor*100);
